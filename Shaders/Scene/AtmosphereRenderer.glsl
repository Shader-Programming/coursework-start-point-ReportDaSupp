...
void main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        float displacement = 1.35;

        float effectiveRadius = displacement;
        vec3 spherePos = normPosition * effectiveRadius;
        vec3 modelPosition = (vec4(spherePos, 1.0)).xyz;

        if (!sphere)
        {
            spherePos = tes_position[i] * 1.35;
...
const vec3 atmosphereColor = vec3(0.52, 0.80, 0.92);

void main() {
    vec3 viewDir = normalize(cameraPos - FragPos);
    float fresnel = pow(1.0 - dot(viewDir, normalize(Normal)), 1.5);
    float heightAboveSurface = length(FragPos) - planetRadius;

    float atmosphereThickness = 25.0;
    float atmosphericFade = clamp(heightAboveSurface / atmosphereThickness, 0.0, 1.0);
    
    vec3 color = atmosphereColor * fresnel * (1.0 - atmosphericFade);
    
    if (heightAboveSurface < 0.0) {
        color += atmosphereColor * 0.75;
    }
    
    FragColor = vec4(color, 1.0 - exp(-fresnel * 3.0 * atmosphericFade));
}