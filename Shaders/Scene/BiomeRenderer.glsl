...
vec3 biomeColor = vec3(0.5, 0.5, 0.5);                                              // Base Color
...
vec3 getBiomeColor(float height, float temperature, float precipitation) {
    ...
    vec3 waterBaseColor = mix(dirtColor, sandColor, smoothstep(0.4, 0.48, height));     // Base Water Color

    if (height < 0.52) {
        float depthFactor = (0.52 - height) / 0.52;
        waterBaseColor = mix(waterBaseColor, dirtColor * 0.5, depthFactor);
        biomeColor = mix(waterBaseColor, sandColor, smoothstep(0.40, 0.52, height));
    } else {
